...
if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/MPS2020.3/mps.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/MPS2020.3/mps.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/MPS2020.3/mps$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/MPS2020.3/mps$BITS.vmoptions"
...
MAIN_CLASS=org.jetbrains.projector.server.ProjectorLauncher
IDEA_PATHS_SELECTOR=MPS2020.3
"$JAVA_BIN" \
  ${AGENT} \
  -classpath "$CLASSPATH" \
  ${VM_OPTIONS} \
  ...
  ${IDE_JVM_ARGS} \
  -Dorg.jetbrains.projector.server.classToLaunch=jetbrains.mps.Launcher \
